...
function updateMessageTimestamp() {
    ...
    localStorage.setItem('lastMessageTimestamp', JSON.stringify(lastMessageTimestamp));
...
function sendMessage() {
    if (!canSendMessage()) {
        console.log('يمكنك إرسال رسالتين فقط كل 4 ساعات!');
        return;
    }
...
function submitIdea() {
    ...
    const newIdea = {
        ...
        idea: ideaText,
        timestamp: new Date().toLocaleString() // تاريخ ووقت الإرسال
    };
